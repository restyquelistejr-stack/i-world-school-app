...
export default function ClassDetailsClient({ initialClassData, classId }: ClassDetailsClientProps) {
  ...
  async function loadClassDetails() {
    setLoading(true);
    try {
      // 1. Load Class Options (The Proposed Schedule)
      ...
      // 2. Load Enrolled Students (via class_enrollments)
  ...
  // ==========================================================
  // GROUP REGISTRATION LOGIC (Fully preserved from your file)
  // ==========================================================
  const addStudentRow = () => {
    setNewStudents([
      ...newStudents,
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto relative">
      ...
      {/* Class Info Grid - Uses initialClassData passed from Server Component */}
      ...
      {/* ========================================================== */}
      {/* GROUP REGISTRATION MODAL (Fully preserved) */}
      {/* ========================================================== */}
      {showAddStudentModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto flex-1">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {newStudents.map((student, index) => (
                      <tr key={index} className="hover:bg-gray-50/50">
                        <td className="p-2">
                          <input type="text" value={student.full_name} onChange={(e) => updateStudentRow(index, 'full_name', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" placeholder="John Doe" />
                        </td>
                        <td className="p-2">
                          <input type="email" value={student.email} onChange={(e) => updateStudentRow(index, 'email', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" placeholder="john@email.com" />
                        </td>
                        <td className="p-2">
                          <input type="text" value={student.phone} onChange={(e) => updateStudentRow(index, 'phone', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2">
                          <select value={student.gender} onChange={(e) => updateStudentRow(index, 'gender', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm bg-white"><option value="">Select</option><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select>
                        </td>
                        <td className="p-2">
                          <input type="text" value={student.nationality} onChange={(e) => updateStudentRow(index, 'nationality', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2">
                          <input type="date" value={student.date_of_birth} onChange={(e) => updateStudentRow(index, 'date_of_birth', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2">
                          <input type="text" value={student.educational_background} onChange={(e) => updateStudentRow(index, 'educational_background', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2">
                          <input type="text" value={student.emergency_contact} onChange={(e) => updateStudentRow(index, 'emergency_contact', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2">
                          <input type="text" value={student.emergency_phone} onChange={(e) => updateStudentRow(index, 'emergency_phone', e.target.value)} className="w-full px-2 py-1.5 border rounded focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                        </td>
                        <td className="p-2 text-center">
                          <button onClick={() => removeStudentRow(index)} className="text-red-400 hover:text-red-600 text-lg font-bold leading-none">✕</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <button onClick={addStudentRow} className="mt-4 w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-sm text-gray-500 hover:border-blue-500 hover:text-blue-600 transition">➕ Add Another Student</button>
            </div>
            ...
            <div className="p-6 border-t border-gray-200 flex justify-end gap-3 bg-gray-50">
              ...
              <button onClick={handleBulkRegister} disabled={submittingStudents} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition disabled:opacity-50 flex items-center gap-2">{submittingStudents ? 'Saving...' : '✅ Register & Enroll All'}</button>
            </div>
